...
We will see a quick setup to configure the ``bigbluebutton`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``bigbluebutton`` formula
`````````````````````````````````````````````````````

Now, we can provides the per DNS domain name configuration files for the ``bigbluebutton`` formulas under ``/srv/salt/bigbluebutton/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/bigbluebutton/parameters/dns:domain/
    ln -s dns:domain /srv/salt/bigbluebutton/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/bigbluebutton/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/bigbluebutton/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``bigbluebutton`` formula
``````````````````````````````````````````````````````

Now, we can provides the per role configuration files for the ``bigbluebutton`` formulas under ``/srv/salt/bigbluebutton/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/bigbluebutton/parameters/roles
...
We create a configuration for the role ``bigbluebutton/server`` in ``/srv/salt/bigbluebutton/parameters/roles/bigbluebutton/server.yaml``:
...
We create another configuration for the role ``bigbluebutton/client`` in ``/srv/salt/bigbluebutton/parameters/roles/bigbluebutton/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles bigbluebutton/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles bigbluebutton/client
...
.. code-block:: console
    ...
    mv /srv/salt/bigbluebutton/parameters/dns:domain/  '/srv/salt/bigbluebutton/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-bigbluebutton-as-json:
      test.nop:
        - name: {{ bigbluebutton | json }}
...
Here is an example based on `template-formula/bigbluebutton/config/file.sls`_:
...
.. code-block:: sls
    ...
    bigbluebutton-config-file-file-managed:
      file.managed:
        - name: {{ bigbluebutton.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='bigbluebutton-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ bigbluebutton.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
          - sls: {{ sls_package_install }}
        - context:
            bigbluebutton: {{ bigbluebutton | json }}

This ``sls`` file expose a ``bigbluebutton`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ bigbluebutton['winner'] }}
...
.. _template-formula/bigbluebutton/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/bigbluebutton/config/file.sls
